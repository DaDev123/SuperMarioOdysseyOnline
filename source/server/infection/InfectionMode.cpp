#include "server/infection/InfectionMode.hpp"
...
#include "server/infection/InfectionPacket.hpp"
...
void InfectionMode::processPacket(Packet* _packet) {
    ...
    if (packet->mUserID == Client::getClientId()) {
        ...
        if (updateType & InfectionUpdateType::STATE) {
            updateTagState(packet->isIt);
        } else if (updateType & InfectionUpdateType::TIME) {
...
Packet* InfectionMode::createPacket() {
    ...
    packet->setUpdateType(static_cast<InfectionUpdateType>(InfectionUpdateType::STATE | InfectionUpdateType::TIME));
    return packet;
}
        ...
        if (mInvulnTime < 5) {
            ...
            if (PlayerFunction::isPlayerDeadStatus(playerBase)) {
                ...
                for (size_t i = 0; i < (size_t)mPuppetHolder->getSize(); i++) {
                    ...
                    if (!other->isConnected || !other->isInSameStage || other->infectionIsHiding() || isYukimaru) {
                        continue;
                    }
...
bool InfectionMode::showNameTag(PuppetInfo* other) {
    return (
        ...
        || (isPlayerSeeking() && other->infectionIsSeeking())
    );
}
...
void InfectionMode::debugMenuControls(sead::TextWriter* gTextWriter) {
    ...
    gTextWriter->printf("- [Infection] ← | Decrease the Timer\n");
    gTextWriter->printf("- [Infection] → | Increase the Timer\n");
    gTextWriter->printf("- [Infection] L + ↓ | Reset the Timer\n");
